...
export class BlogController {
    constructor(readonly blogService:BlogService,
                readonly blogQueryRepository:BlogQueryRepository,
    ...
    async getPostsByBlog(req: Request, res: Response){
        ...
        if (!blog) {
            ...
            const answer = await this.postQueryRepository.getMany(sanitizedQuery, blogId);
            if (!answer) {
                return res.sendStatus(500)
            }
    ...
    async createPostByBlog(req: Request, res: Response){
        ...
        try {
            ...
            if (!blog) {
                ...
                const createdPost=await this.postQueryRepository.findForOutput(new ObjectId(createdPostId.id))
                if (!createdPost) {
