...
export class PostController {
    ...
    async createCommentByPost(req: Request, res: Response){
        ...
        const createdComment=await this.commentQueryRepository.findForOutput(new ObjectId(createdCommentId.id))
        if(createdComment){
    ...
    async getCommentsByPost(req: Request<{ postId: string }>, res: Response){
        ...
        const comments=await this.commentQueryRepository.getMany(sanitizedQuery, req.params.postId)
        if(!comments){
            res.sendStatus(404)
            return
        }
