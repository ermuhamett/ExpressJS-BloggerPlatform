...
describe('/comments', () => {
    ...
    describe('Create user for comments and take token', () => {
    ...
    describe('POST requests for comments', () => {
        ...
        it('should return error if we dont send access token', async () => {
            await agent(app)
                .post(`${SETTINGS.PATH.POSTS}/${postId}${SETTINGS.PATH.COMMENTS}`)
                .send(contentData)
                .expect(401);
    ...
    describe('DELETE requests for comments', () => {
        it('should return error if not set token', async () => {
            await agent(app)
                .put(`${SETTINGS.PATH.COMMENTS}/${commentId}`)
                .send(contentData)
                .expect(401)
